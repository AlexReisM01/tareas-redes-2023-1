...
def parse_packet(IP_packet: bytes):
    pckt = IP_packet.split(b',', 3)
    parsed_IP_packet = {"ip": pckt[0].decode(), "port": pckt[1].decode(),"TTL": int(pckt[2].decode())  ,"msg": pckt[3]}
    return parsed_IP_packet

def create_packet(parsed_IP_packet: dict):
    packet = parsed_IP_packet["ip"].encode() + b',' + parsed_IP_packet["port"].encode() + b',' + str(parsed_IP_packet["TTL"]).encode() + b',' + parsed_IP_packet["msg"]
    return packet
        ...
        while(True):
            ...
            if parsed_packet["ip"] == self.ip and int(parsed_packet["port"]) == self.port:
                ...
                if next_hop_IP==0 and next_hop_port==0:
                    ...
                    self.routerSocket.sendto(create_packet(parsed_packet), (next_hop_IP, next_hop_port))
